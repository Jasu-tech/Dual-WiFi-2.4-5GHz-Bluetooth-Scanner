...
// Based on actual touch measurements (extrapolated to screen edges)
// Measured at 4 points: Banner(Y≈25, touchY=1864), 2.4GHz(Y≈95, touchY=1414), 
//                       5GHz(Y≈185, touchY=837), BLE(Y≈275, touchY=357)
// touchX average at center: 989 (range 929-1050)
// Touch calibration values from 4-point calibration (z1 threshold=80)
...
// Direct mapping - PORTRAIT mode, no axis swap
void mapTouchToScreen(int touchX, int touchY, int& screenX, int& screenY) {
  // PORTRAIT mode (240x320, pystysuuntainen) with 180° display rotation
  // Direct mapping: touchX→screenX, touchY→screenY
  // Only Y inverted for 180° display rotation (X stays normal)
...
void handleTouchInput() {
  ...
  // UP/DOWN buttons - ONLY work in list views, ALWAYS prevent other actions when button area touched
  if ((isUpButton || isDownButton) && (currentView == VIEW_WIFI_LIST || currentView == VIEW_BLE_LIST)) {
    ...
    return;  // ALWAYS return when button area is touched in list view - prevents detail view opening
  }
  
  // Menu navigation - ONLY when in menu view (4-section layout)
  // Now with CORRECT mapping, screenY matches drawn Y positions!
  if (currentView == VIEW_MENU) {
    Serial.print("Menu touch - screenY: ");
    Serial.println(screenY);
    
    // Banner (Y: 0-50) - ignore, no action
    if (screenY < 50) {
      Serial.println("-> Banner (no action)");
      return;
    }
    
    // Section 2: 2.4GHz WiFi (Y: 50-140)
    ...
    // Section 3: 5GHz WiFi (Y: 140-230)
    if (screenY >= 140 && screenY < 230) {
      Serial.println("-> 5GHz WiFi");
      isScan24GHz = false;
      isScan5GHz = true;
      startWiFiScan();
      return;
  ...
  // WiFi detail view
    ...
    // Any tap (except buttons) = back to list
  ...
  // WiFi/BLE list - tap in center area to show details
  // Only if NOT on button areas
...
void setup() {
  ...
  // Initialize display
  Serial.println("Initializing display with Hardware SPI @ 40MHz...");
  tft->begin(40000000);
  tft->invertDisplay(true);  // FIX: Correct color inversion!
  ...
  // Initialize touch screen (Software SPI)
  Serial.println("Initializing XPT2046 touch screen (Software SPI)...");
  touchSPIBegin();
  Serial.println("XPT2046 Software SPI initialized!");
...
void drawWiFiList() {
  ...
  // UP button (top-right, wider for better touch response)
  ...
  // DOWN button (bottom-right, same size as UP button)
...
void startWiFiScan() {
  ...
  // Start WiFi scan using callback handler
  if (wifi_scan_networks(wifiScanResultHandler, NULL) == RTW_SUCCESS) {
    // Wait for scan to complete (callback sets wifiScanComplete)
    uint8_t attempts = 20;
  ...
  if (isScan24GHz) Serial.print("2.4GHz only");
  else if (isScan5GHz) Serial.print("5GHz only");
  else Serial.print("All bands");
  Serial.print("): ");
...
void startBLEScan() {
  ...
  Serial.print("BLE scan complete: ");
...
void drawBLEDetail() {
  ...
  if (bleDevices[selectedIndex].hasManufacturerData) {
    ...
    // Show manufacturer name if known
    String mfgName = getManufacturerName(bleDevices[selectedIndex].manufacturerID);
    if (mfgName.length() > 0) {
      // Known manufacturer - show name on first line
      tft->setCursor(20, y + 15);
      tft->setTextColor(COLOR_YELLOW);
      tft->setFont(u8g2_font_helvR10_tr);
      tft->print(mfgName);
      // Show ID and length on second line
      ...
      // Unknown manufacturer - show just ID and length
      tft->setCursor(20, y + 15);
      tft->setTextColor(COLOR_CYAN);
      tft->setFont(u8g2_font_helvR08_tr);
      tft->print(bleDevices[selectedIndex].manufacturerData);
  ...
  if (bleDevices[selectedIndex].hasServices) {
    ...
    // Split long UUID list if needed
